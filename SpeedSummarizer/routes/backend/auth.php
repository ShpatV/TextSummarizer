...
use App\Domains\Auth\Http\Controllers\Backend\Files\FilesController;
...
Route::group([
    ...
    Route::group([
        ...
        Route::get('/', [FilesController::class, 'index'])
            ...
            ->breadcrumbs(function (Trail $trail) {
                $trail->parent('admin.dashboard')
                ->push(__('Files'), route('admin.auth.files.index'));
            });

        Route::get('/create', [FilesController::class, 'create'])
            ->name('create')
            ->breadcrumbs(function (Trail $trail) {
                $trail->parent('admin.auth.files.index')
                    ->push(__('Create files'), route('admin.auth.files.create'));
            });

        Route::post('/files', [FilesController::class, 'files'])->name('files');

        Route::group(['prefix' => '{id?}' ], function () {
            Route::get('/', [FilesController::class, 'show'])
                    ->name('show')
                    ->breadcrumbs(function (Trail $trail,$files_id) {
                        $trail->parent('admin.auth.files.index')
                            ->push(__('Show Files'), route('admin.auth.files.show',$files_id));
                });
            Route::delete('/', [FilesController::class, 'destroy'])->name('destroy');
        });
    });
});
